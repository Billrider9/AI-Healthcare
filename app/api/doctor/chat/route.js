...
/** Default model — override with GEMINI_MODEL if your key/region requires another ID (e.g. gemini-2.0-flash). */
...
function textFromGeminiResponse(response) {
  try {
    ...
    if (block) {
      throw new Error(`Response blocked (${block}). Try rephrasing your message.`);
    }
    throw new Error('No text returned from the model');
  }
}

// Generate content via official SDK (avoids v1 REST / model mismatch issues)
async function generateContentDirect(prompt, contextMessages = []) {
  ...
  const history = normalizeChatHistory(contextMessages);

  if (history.length === 0) {
    const result = await model.generateContent(prompt);
    return textFromGeminiResponse(result.response);
  }
...
- Include a disclaimer about seeking professional medical advice`;
      ...
      console.log(`Successfully generated AI doctor response (${GEMINI_MODEL})`);
      ...
      const detail =
        process.env.NODE_ENV === 'development' && aiError?.message
          ? aiError.message
